...
class LoggerNew:
 def __init__(self,flag,clevel = logging.DEBUG,Flevel = logging.DEBUG):
  ...
  sh.setLevel(clevel)
  #设置文件日志
  fh = logging.FileHandler('mylog')
  fh.setFormatter(fmt)
  fh.setLevel(Flevel)
  # 添加TimedRotatingFileHandler
  # 定义一个10秒换一次log文件的handler
  # 保留3个旧log文件
  timefilehandler = logging.handlers.TimedRotatingFileHandler('mylog.log', when='S', interval=10, backupCount=3)
