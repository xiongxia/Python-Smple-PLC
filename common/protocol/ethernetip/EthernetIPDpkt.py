...
class EtherNetIPSocket(object):
    ...
    def mkReqPath(self, clas, inst, attr):
        ...
        if clas > 255: 
            ...
            print("clas_data:"+str(clas_data))

        inst_data = struct.pack("BB", 0x24, inst)
        print("inst_data:"+str(inst_data))
    ...
    def listInterfaces(self):
        ...
        pkt = EncapsulationPacket(command=EncapsulationPacket.ENCAP_CMD_LISTINTERFACES,
    ...
    def unconnSend(self, service, data, context=0, chk=0, chkdata="\x00"):
        ...
        if len(inp) != 0:
            ...
            if len(data) > 0:
                ...
                if pkt.status == EncapsulationPacket.ENCAP_STATUS_SUCCESS and pkt.command == EncapsulationPacket.ENCAP_CMD_SENDRRDATA:
